...
## Validation Stance
- Do not treat existing section-view output as source-of-truth for Babylon migration.
- Babylon path should consume canonical profile primitives/loops and be validated independently from legacy SVG composition.
...
## Execution Log
- 2026-02-11: BAB-006..BAB-010 completed.
- Implemented Babylon runtime with deterministic camera/lighting and canonical-loop mesh rendering.
- Added semantic material mapping and Babylon diagnostics surfacing in the bushing drafting UI.
- Added Babylon smoke coverage in `tests/bushing-e2e-smoke-ui.spec.ts`.
- Validation: `npm run -s verify:bushing-architecture`, `npm run -s verify:bushing-e2e-smoke`, and `npm run -s verify:bushing-regression` all passed.
- 2026-02-11: BAB-011..BAB-020 completed.
- Added parity harness (`BushingBabylonParity.ts`), countersink parity coverage, Babylon viewport controls, export compatibility tests, and dual-path regression wiring.
- Added ADR for export source-of-truth during migration (`ADR_BAB_015_EXPORT_SOURCE_OF_TRUTH.md`).
- 2026-02-11: BAB-021..BAB-030 completed for bushing scope.
- Default renderer switched to Babylon with explicit SVG fallback retained.
- Removed D3 from active bushing drafting path (`sectionProfile.ts`, `d3Primitives.ts`) and retired `d3BushingFigure*` modules.
- Caveat: BAB-025 remains blocked outside bushing scope because `d3` is still required by Surface/Profile modules.
- Closeout report: `implementation/bushing-d3-to-babylon-plan-v1/BABYLON_BATCH2_3_REPORT.md`.
