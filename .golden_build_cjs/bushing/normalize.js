...
function normalizeToleranceMode(v) {
    const s = String(v ?? '').toLowerCase();
    return s === 'limits' ? 'limits' : 'nominal_tol';
}
function normalizeEndConstraint(v) {
    const s = String(v ?? 'free').toLowerCase();
    if (s === 'one_end' || s === 'one-end' || s === 'one end')
        return 'one_end';
    if (s === 'both_ends' || s === 'both-ends' || s === 'both ends')
        return 'both_ends';
    return 'free';
}
...
function normalizeCapabilityMode(v) {
    const s = String(v ?? '').toLowerCase();
    if (s === 'reamer_fixed' || s === 'reamer-fixed' || s === 'reamer fixed')
        return 'reamer_fixed';
    if (s === 'adjustable')
        return 'adjustable';
    return 'unspecified';
...
function normalizeBushingInputs(raw) {
    ...
    const policyRaw = (raw.interferencePolicy ?? raw.interference_policy ?? {});
    const capabilityRaw = (raw.boreCapability ?? raw.bore_capability ?? {});
    const interferencePolicy = {
        enabled: normalizeBool(policyRaw.enabled ?? raw.enforceInterferenceTolerance ?? raw.enforce_interference_tolerance, false),
        lockBore: normalizeBool(policyRaw.lockBore ?? policyRaw.lock_bore ?? raw.lockBoreForInterference ?? raw.lock_bore_for_interference, true),
