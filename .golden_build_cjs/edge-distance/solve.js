...
exports.evaluateEdgeDistance = evaluateEdgeDistance;
const isNum = (n) => typeof n === 'number' && Number.isFinite(n);
const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));
function solveEdgeDistance({ d, t, P, thetaDeg, Fbru, Fsu, shearPlaneModel, shearPlaneDeg }) {
    ...
    const governing = (isNum(ed_strength) && isNum(ed_sequence))
...
function evaluateEdgeDistance(params) {
    ...
    const shearPlaneModel = params.shearPlaneModel;
    const shearPlaneDeg = isNum(params.shearPlaneDeg) ? Number(params.shearPlaneDeg) : undefined;
