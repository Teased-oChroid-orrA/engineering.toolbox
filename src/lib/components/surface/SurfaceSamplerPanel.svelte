<script lang="ts">
  type SamplerMode = 'quad' | 'edges';

  let {
    samplerAppend = $bindable(false),
    samplerMode = $bindable<SamplerMode>('quad'),
    samplerNu = $bindable(12),
    samplerNv = $bindable(12),
    samplerEdgeSegs = $bindable(8),
    samplerErr = $bindable<string | null>(null),
    onGenerate
  } = $props<{
