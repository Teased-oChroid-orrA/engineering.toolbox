  ...
  $: guidance = (() => {
    if (results.warningCodes?.length) {
      return {
        ...
        lines: results.warningCodes.map((w) => w.message).slice(0, 2)
      };
    }
...
<div
  class={`glass-card bushing-pop-card rounded-xl border p-3 text-xs ${guidance.tone === 'warning' ? 'border-amber-400/30 bg-amber-500/10' : 'border-teal-400/25 bg-teal-500/10'}`}>
  ...
  <div class="mt-2 space-y-1.5">
    {#each guidance.lines as line}
      <div class="rounded-md border border-white/10 bg-black/20 px-2 py-1.5 bushing-pop-sub text-white/80">{line}</div>
    {/each}
  </div>
</div>
