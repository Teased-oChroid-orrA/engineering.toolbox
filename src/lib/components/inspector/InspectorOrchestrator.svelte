<script lang="ts">
  ...
  import { setupShowDataControlsEffect, setupSliceFetchEffect, setupReactiveFilterEffect, setupCrossQueryEffect, setupPersistStateEffect, setupUiAnimDurEffect, setupQuietBackendLogsEffect, setupContextMenuEffect, setupCategoryColumnChangeEffect, setupCategorySearchEffect } from '$lib/components/inspector/InspectorOrchestratorEffects.svelte';
  import { filterControllerCtx as buildFilterControllerCtx, loadControllerCtxCore as buildLoadControllerCtxCore, loadControllerCtxStateMain as buildLoadControllerCtxStateMain, loadControllerCtxStateQueryAndGrid as buildLoadControllerCtxStateQueryAndGrid, loadControllerCtx as buildLoadControllerCtx, gridControllerCtx as buildGridControllerCtx, rowDrawerControllerCtx as buildRowDrawerControllerCtx, modalUiCtx as buildModalUiCtx, recipesControllerCtx as buildRecipesControllerCtx, schemaControllerCtx as buildSchemaControllerCtx, schemaInsightsCtx as buildSchemaInsightsCtx, stateControllerCtx as buildStateControllerCtx } from '$lib/components/inspector/InspectorOrchestratorContexts';
    ...
    let schemaDrift = $derived.by(() => computeSchemaDrift(schemaStats ?? [], schemaDriftBaseline ?? null)); let sortColIdx = $state<number | null>(null); let sortDir = $state<'asc' | 'desc'>('asc'); let sortSpecs = $state<{ colIdx: number; dir: 'asc' | 'desc' }[]>([]); let sortPriority = $derived.by(() => { const out: Record<number, number> = {}; for (let i = 0; i < (sortSpecs ?? []).length; i++) out[sortSpecs[i].colIdx] = i; return out; }); let pinnedLeft = $state<number[]>([]); let pinnedRight = $state<number[]>([]); let hiddenColumns = $state<number[]>([]); let columnWidths = $state<Record<number, number>>({}); let showShortcuts = $state(false); let quietBackendLogs = $state(true);
  ...
  function loadControllerCtxCore() { return buildLoadControllerCtxCore({ invoke, debugLogger, dialogMod, fnv1a32, heuristicHasHeaders, computeDatasetIdentity, upsertWorkspaceDatasetInList, loadRecipesForDataset, loadLastStateForDataset, applyState, runFilterNow, buildFilterSpec, queueDebug, activateWorkspaceDataset }); }
  function loadControllerCtxStateMain() { return buildLoadControllerCtxStateMain({ hiddenUploadInput, isLoading, isMergedView, loadError, hasHeaders, headerMode, headerHeuristicReason, pendingText, pendingPath, showHeaderPrompt, headers, totalRowCount, totalFilteredCount, visibleRows, colTypes, datasetId, datasetLabel, recipes, pendingRestore, hasLoaded, showDataControls, activeDatasetId }); }
  function loadControllerCtxStateQueryAndGrid() { return buildLoadControllerCtxStateQueryAndGrid({ loadedDatasets, query, matchMode, targetColIdx, numericF, dateF, catF, suspendReactiveFiltering, sortColIdx, sortDir, sortSpecs, visibleColumns, pinnedLeft, pinnedRight, hiddenColumns, columnWidths, crossQueryBusy, queryScope, crossQueryResults, mergedHeaders, mergedRowsAll, preMergedHeaders, preMergedColTypes, preMergedTotalRowCount, preMergedTotalFilteredCount }); }
  function loadControllerCtx() { return { ...loadControllerCtxCore(), ...loadControllerCtxStateMain(), ...loadControllerCtxStateQueryAndGrid() }; }
  ...
  function gridControllerCtx() { return buildGridControllerCtx({ invoke, recordPerf, hasLoaded, sliceGate, startIdx: gridWindow.startIdx, endIdx: gridWindow.endIdx, visibleColIdxs, isMergedView, mergedRowsAll, visibleRows, loadError, totalFilteredCount, sliceTimer, headers, sortGate, sortColIdx, sortDir, sortSpecs, visibleColumns, columnPickerNotice, showColumnPicker, hiddenColumns, pinnedLeft, pinnedRight, columnWidths }); }
  ...
  function recipesControllerCtx() { return buildRecipesControllerCtx({ RECIPES_STORE_KEY, LEGACY_RECIPES_STORE_KEYS, LAST_STATE_STORE_KEY, LEGACY_LAST_STATE_KEYS, loadRecipesForDatasetFromStore, persistRecipesForDatasetToStore, loadLastStateForDatasetFromStore, persistLastStateForDatasetToStore, buildRecipeExportBlob, mergeImportedRecipes, toCsvText, downloadText, newRecipeId, captureState, applyState, persistRecipesForDataset, autoRestoreEnabled, datasetId, datasetLabel, recipes, recipeNotice, hasLoaded, headers, visibleRows, visibleColIdxs, totalFilteredCount, activeFilterHash, schemaStats, perf, invoke, recipeName, recipeTags }); }
  ...
  function modalUiCtx() { return buildModalUiCtx({ withViewTransition, modalPos, dragState, recipeNotice, showRecipeModal, showShortcuts, showSvarBuilder, showRegexGenerator, genTab }); }
...
<div class="flex flex-col gap-3 inspector-lab inspector-reveal">
  ...
  <InspectorTopControlsPanel
    ...
    onOpenRecipes={() => openRecipesModal(modalUiCtx())} onOpenStreamLoad={async () => await openStreamLoadFromMenuController(loadControllerCtx())} onOpenFallbackLoad={() => openFallbackLoadFromMenuController(loadControllerCtx())}
    onMultiQueryEnabledChange={onMultiQueryEnabledChange} onMultiQueryExpandedChange={onMultiQueryExpandedChange} onAddMultiQueryClause={onAddMultiQueryClause}
  ...
  <InspectorOverlayPanel
    ...
    toggleVisibleCol={(i: number) => toggleVisibleColController(gridControllerCtx(), i)} saveCurrentAsRecipe={() => saveCurrentAsRecipeController2(recipesControllerCtx())} exportRecipesCurrent={async () => await exportRecipesCurrentController(recipesControllerCtx())}
