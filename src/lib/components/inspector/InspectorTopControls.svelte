<script lang="ts">
  ...
  import TopControlsLayout from '$lib/components/inspector/TopControlsLayout.svelte';
...
<div class="glass-panel rounded-2xl p-4 border border-white/10" data-testid="inspector-top-controls">
  <div use:aa>
    <TopControlsLayout
      spans={topControlSpans}
      headersSnippet={headersSlot}
      targetSnippet={targetSlot}
      matchSnippet={matchSlot}
      scopeSnippet={scopeSlot}
      querySnippet={querySlot}
      optionsSnippet={optionsSlot}
      maxScanSnippet={maxScanSlot}
    >
      {#snippet headersSlot()}
        <ControlCard label="Headers">
          <select
            ...
            value={headerMode}
            disabled={isLoading}
            onchange={(e) => onHeaderModeChange((e.currentTarget as HTMLSelectElement).value as HeaderMode)}
          >
      ...
      {#snippet matchSlot()}
        <ControlCard label="Match">
          <select
            ...
            disabled={!hasLoaded}
            value={matchMode}
            onchange={(e) => onMatchModeChange((e.currentTarget as HTMLSelectElement).value as MatchMode)}
          >
      ...
      {#snippet scopeSlot()}
        <ControlCard label="Query Scope">
          <select
            ...
            value={queryScope}
            title="Query scope"
            onchange={(e) => onQueryScopeChange((e.currentTarget as HTMLSelectElement).value as QueryScope)}
          >
      ...
      {#snippet querySlot()}
        <ControlCard label="Query">
          <div class="relative">
            <input
              class="input input-sm w-full glass-input pr-28"
              disabled={!hasLoaded}
              placeholder={matchMode === 'regex' ? 'Regex pattern…' : 'Type to filter…'}
              value={query}
      ...
      {#snippet optionsSlot()}
        <ControlCard label="Tier-2">
          <div class="glass-panel rounded-xl border border-white/10 px-2 min-h-[38px] flex items-center justify-between gap-2">
            <button
              class="btn btn-sm variant-soft flex items-center gap-2"
              disabled={!hasLoaded}
              title="Pick visible columns"
            ...
            <label class="flex items-center gap-2 text-[12px] text-white/85 select-none">
              ...
              <input
                ...
                checked={tier2Open}
                disabled={!hasLoaded}
                title="Toggle Tier-2 filters"
                onchange={(e) => onTier2Toggle((e.currentTarget as HTMLInputElement).checked)}
              />
            </label>
          </div>
        </ControlCard>
      ...
      {#snippet maxScanSlot()}
        <ControlCard label="Max Scan">
          <div class="relative group z-50">
            <input
              class="input input-sm w-full glass-input"
              disabled={!hasLoaded}
              placeholder="MAX SCAN"
              value={maxRowsScanText}
              oninput={(e) => onMaxRowsScanTextChange((e.currentTarget as HTMLInputElement).value)}
            />
