...
type CalcState = {
  ...
  enforcementSatisfied: boolean;
  enforcementBlocked: boolean;
  enforcementReasonCodes: InterferenceEnforcementReasonCode[];
  enforcementLowerViolation: number;
  enforcementUpperViolation: number;
...
export function computeState(input: ReturnType<typeof normalizeBushingInputs>): CalcState {
  const matHousing = getMat(input.matHousing);
  const matBushing = getMat(input.matBushing);

  let boreTol = resolveTolerance({
  ...
  let odFit = buildOdTolerance(boreTol, interferenceTol);
  ...
  const enforcementReasonCodes: InterferenceEnforcementReasonCode[] = [];
  ...
  if (!enforceInterference) enforcementReasonCodes.push('ENFORCEMENT_DISABLED');
  if (enforceInterference && odFit.status === 'infeasible') {
    if (lockBore) {
      ...
      if (enforcedBore.note) {
        ...
        if (enforcedBore.note.includes('capability floor')) enforcementReasonCodes.push('BLOCKED_CAPABILITY_FLOOR');
      }
  ...
  const vio = containmentViolations(odFit.achievedInterference, interferenceTol);
  const enforcementSatisfied = !enforceInterference || (vio.lowerViolation <= EPS && vio.upperViolation <= EPS);
...
export function buildWarnings(validationWarnings: BushingWarning[], s: CalcState): { warningCodes: BushingWarning[]; warnings: string[] } {
...
export function toOutput(s: CalcState, warningCodes: BushingWarning[], warnings: string[]): BushingOutput {
