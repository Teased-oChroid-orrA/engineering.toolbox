import type { BearingSegment, UniversalBearingResult } from './types';

// Ported 1:1 from legacy App.Logic.Bearing.calculateUniversalBearing
export function calculateUniversalBearing(profile: BearingSegment[]): UniversalBearingResult {
  ...
  const results = profile.map((segment) => {
    ...
    const geometry: 'CYLINDER' | 'FRUSTUM' = isCyl ? 'CYLINDER' : 'FRUSTUM';
    return {
      ...segment,
      eta,
      rawArea,
